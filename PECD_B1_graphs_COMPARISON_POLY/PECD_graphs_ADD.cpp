...
void load_TH1D(TFile* S_LFile, TFile* S_RFile, TFile* R_LFile, TFile* R_RFile, TH1D** S_LCP_TH1D_Array, TH1D** S_RCP_TH1D_Array, TH1D** R_RCP_TH1D_Array, TH1D** R_LCP_TH1D_Array, TH1D** general_Array, TString* dirlist, TString* histlist1D, const char dir[]) {
	...
	TCanvas* c56 = new TCanvas("all TH1D histos", "all = 12");
...
void load_TH2D(TFile* S_LFile, TFile* S_RFile, TFile* R_LFile, TFile* R_RFile, TH2D** S_LCP_TH2D_Array, TH2D** S_RCP_TH2D_Array, TH2D** R_LCP_TH2D_Array, TH2D** R_RCP_TH2D_Array, TH2D** general2D_Array, TH2D** addSL_RR_2D_Array, TH2D** addSR_RL_2D_Array, TString* dirlist, TString* histlist2D,  TString* namelist2D, const char dir[]) {
	...
	for(int i=0;i<1;i++) {
		...
		for (int j=0;j<1;j++) {
			...
			//add option addSR_RL_2D_Array

			if(S_RCP_TH2D_Array[j+i*3]->Add(R_LCP_TH2D_Array[j+i*3]) == kFALSE) {
				std::cout << S_LCP_TH2D_Array[j+i*3]->GetName() << ": Failed to sum!" << "\n";
				break;
			}
			else
				std::cout << S_LCP_TH2D_Array[j+i*3]->GetName() << " + " << R_RCP_TH2D_Array[j+i*3]->GetName() << ": DONE!" << "\n";
			addSR_RL_2D_Array[j+i*3] = S_LCP_TH2D_Array[j+i*3];
			addSR_RL_2D_Array[j+i*3]->SetName(dirlist[i]+"/"+histlist2D[j]);

			if(S_LCP_TH2D_Array[j+i*3]->Add(R_RCP_TH2D_Array[j+i*3]) == kFALSE) {
				std::cout << S_LCP_TH2D_Array[j+i*3]->GetName() << ": Failed to sum!" << "\n";
				break;
			}
			else
				std::cout << S_LCP_TH2D_Array[j+i*3]->GetName() << " + " << R_RCP_TH2D_Array[j+i*3]->GetName() << ": DONE!" << "\n";
			addSL_RR_2D_Array[j+i*3] = S_LCP_TH2D_Array[j+i*3];
			addSL_RR_2D_Array[j+i*3]->SetName(dirlist[i]+"/"+histlist2D[j]);

			std::cout << addSR_RL_2D_Array[j+i*3]->GetName() << ": normalized with # entries: " << addSR_RL_2D_Array[j+i*3]->GetEntries() << "\n \n";
...

{
	...
	// modification for the adding
	TH2D** addSL_RR_2D_Array;
	addSL_RR_2D_Array = new TH2D*[15];

	TH2D** addSR_RL_2D_Array;
	addSR_RL_2D_Array = new TH2D*[15];
	...
	/*TFile* S_LFile = new TFile("E:/Soleil_2018_09_SEX/Analysis/S-C3H3F3O_550eV_CL_10800-2350ns_analysis.root", "READ");
	if (S_LFile->IsOpen()) printf("%s: File opened successfully!\n", S_LFile->GetName());
	TFile* S_RFile = new TFile("E:/Soleil_2018_09_SEX/Analysis/S-C3H3F3O_550eV_CR_10800-2350ns_analysis.root", "READ");
	if (S_RFile->IsOpen()) printf("%s: File opened successfully!\n", S_RFile->GetName());
	TFile* R_LFile = new TFile("E:/Soleil_2018_09_SEX/Analysis/R-C3H3F3O_550eV_CL_10800-2350ns_analysis.root", "READ");
	if (R_LFile->IsOpen()) printf("%s: File opened successfully!\n", R_LFile->GetName());
	TFile* R_RFile = new TFile("E:/Soleil_2018_09_SEX/Analysis/R-C3H3F3O_550eV_CR_10800-2350ns_analysis.root", "READ");
	if (R_RFile->IsOpen()) printf("%s: File opened successfully!\n", R_RFile->GetName());*/

	/*TFile* S_LFile = new TFile("E:/Soleil_2018_09_SEX/Analysis/S-C3H3F3O_550eV_CL_9600-3700ns_analysis.root", "READ");
	if (S_LFile->IsOpen()) printf("%s: File opened successfully!\n", S_LFile->GetName());
	TFile* S_RFile = new TFile("E:/Soleil_2018_09_SEX/Analysis/S-C3H3F3O_550eV_CR_9600-3700ns_analysis.root", "READ");
	if (S_RFile->IsOpen()) printf("%s: File opened successfully!\n", S_RFile->GetName());
	TFile* R_LFile = new TFile("E:/Soleil_2018_09_SEX/Analysis/R-C3H3F3O_550eV_CL_9600-3700ns_analysis.root", "READ");
	if (R_LFile->IsOpen()) printf("%s: File opened successfully!\n", R_LFile->GetName());
	TFile* R_RFile = new TFile("E:/Soleil_2018_09_SEX/Analysis/R-C3H3F3O_550eV_CR_9600-3700ns_analysis.root", "READ");
	if (R_RFile->IsOpen()) printf("%s: File opened successfully!\n", R_RFile->GetName());*/

	TFile* S_LFile = new TFile("E:/Soleil_2018_09_SEX/Analysis/S-C3H3F3O_550eV_CL_11800-1400ns_newEfield_phimod_limit.root", "READ");
	if (S_LFile->IsOpen()) printf("%s: File opened successfully!\n", S_LFile->GetName());
	TFile* S_RFile = new TFile("E:/Soleil_2018_09_SEX/Analysis/S-C3H3F3O_550eV_CR_11800-1400ns_newEfield_phimod_limit.root", "READ");
	if (S_RFile->IsOpen()) printf("%s: File opened successfully!\n", S_RFile->GetName());
	TFile* R_LFile = new TFile("E:/Soleil_2018_09_SEX/Analysis/R-C3H3F3O_550eV_CL_11800-1400ns_newEfield_phimod_limit.root", "READ");
	if (R_LFile->IsOpen()) printf("%s: File opened successfully!\n", R_LFile->GetName());
	TFile* R_RFile = new TFile("E:/Soleil_2018_09_SEX/Analysis/R-C3H3F3O_550eV_CR_11800-1400ns_newEfield_phimod_limit.root", "READ");
	if (R_RFile->IsOpen()) printf("%s: File opened successfully!\n", R_RFile->GetName());

	//FILE NAME
	TFile *f = new TFile("FMtox_ID1_m41_69_philimit.root","RECREATE");
	if (f->IsOpen()) {
		//TFile *f = new TFile("FMtox_ID3_m41_69.root", "RECREATE");
		printf("File CREATED successfully\n");
	}
	const char dir[] = {"angular_distr_el/ID1_mol_e0_valid"};

	// reduced PHI
	TString dirlist[1] = {"/EN_gate"};
	TString histlist1D[3] = {"cos(theta)y_e[0]","cos(theta)_e[0]_redPHI","cos(theta)0_Ppol"};
	TString namelist2D[1] = {"/EN_gate"};
	TString histlist2D[2] = {"PECD_e[0](theta)", "PECD_e[0](theta)_redPHI"};
	...
	TCanvas *canv[10]; // dim(cnam + TH2D graphs)
	...
	load_TH2D(S_LFile, S_RFile, R_LFile, R_RFile, S_LCP_TH2D_Array, S_RCP_TH2D_Array, R_LCP_TH2D_Array, R_RCP_TH2D_Array, general2D_Array, addSL_RR_2D_Array, addSR_RL_2D_Array, dirlist, histlist2D, namelist2D, dir);
	std::cout << "load_all TH2D : DONE! \n \n";

	// JUST EN_gate
	for (int i = 0; i < 7; i++) { // dim(cnam NON ASYMM + TH2D graphs)
		if (i < 3) { // TH1D
		...
		else if (i < 5) { // TH2D just asymmetry plot for the PECD map
			int z = i - 3;  //i minus the number of iteration non TH2D (previous TH1D) 
			canv[z + 6] = new TCanvas(cname[z + 6], cname[z + 6]);  // 6 is the number of previous canvas TH1D
			canv[z + 6]->SetCanvasSize(1920, 1080);
			canv[z + 6]->SetWindowSize(1920 + 4, 1080 + 28);
			canv[z + 6]->Divide(4, 4);
			canv[z + 6]->SetFillStyle(4000);
			gStyle->SetOptTitle(0);
			...
			for (int j = 0; j < 4; j++) {
				...
				for (int k = 0; k < 4; k++) {
					...
					canv[z + 6]->cd(counter_2D);
					ratioTH2D[ratio_counter_2D] = function_ratio_TH2D(general2D_Array[j + z * 4], general2D_Array[k + z * 4], cname[z + 6]);
					...
					//ratioTH2D[ratio_counter_2D]->Draw("COLZ2");
			...
			canv[z + 6]->Write();
			printf("plotting and saving TH2D normal canvas %s: DONE! \n \n", cname[z + 6]);
		}
		else if (i < 7) { //TH2D smooth graphs
			int z = i - 5;  //i minus the number of iteration TH1D and TH2D non smoothed
			canv[z + 8] = new TCanvas(cname[z + 8], cname[z + 8]);
			canv[z + 8]->SetCanvasSize(1920, 1080);
			canv[z + 8]->SetWindowSize(1920 + 4, 1080 + 28);
			canv[z + 8]->Divide(4, 4);
			canv[z + 8]->SetFillStyle(4000);
			gStyle->SetOptTitle(0);
			...
			for (int j = 0; j < 4; j++) {
				...
				for (int k = 0; k < 4; k++) {
					sprintf(tempk, "%s", general2D_Array[k + z * 4]->GetTitle());
					canv[z + 8]->cd(counter_2D);
					ratioTH2D[ratio_counter_2D] = function_ratio_TH2D(general2D_Array[j + z * 4], general2D_Array[k + z * 4], cname[z + 8]);
					//ratioTH2D[ratio_counter_2D]->SetMarkerStyle(20);
					TPaveStats *sk = (TPaveStats*)general2D_Array[k + z * 4]->GetListOfFunctions()->FindObject("stats");
					//sk->SetOptStat(11);
					ratioTH2D[ratio_counter_2D]->GetZaxis()->SetRangeUser(-0.15, 0.15);
