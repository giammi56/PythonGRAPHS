...
//         14          2+1+1 + 1   1+2+1 + 1   1+1+2 + 1

//         15          2+1+0 + 1   2+0+1 + 1   1+2+0 + 1  risky
//                     1+0+2 + 1   0+2+1 + 1   0+1+2 + 1  risky

//         16          1+1+1 + 1                          risky

//         17          2+1+1 + 0   1+2+1 + 0   1+1+2 + 0  risky
//         18          1+1+1 + 0                          risky
//         19          2+1+0 + 0   2+0+1 + 0   1+2+0 + 0  risky
//                     1+0+2 + 0   0+1+2 + 0   0+2+1 + 0  risky
//         20          1+1+0 + 1						  risky
...
public:
    ...
    ~signal_corrector_class();	// FUNCTION
    bool clone(signal_corrector_class *);	// FUNCTION
...
{
public:
	nonlinearity_calibration_class();  // CONSTRUCTOR
    ...
    bool clone(nonlinearity_calibration_class *);	// FUNCTION
...
{
public:
	sum_walk_calibration_class();  // CONSTRUCTOR
    ...
    bool clone(sum_walk_calibration_class *);	// FUNCTION
...
public:
	scalefactors_calibration_class();  // CONSTRUCTOR
    ...
    bool clone(scalefactors_calibration_class *);  // FUNCTION
...
public:
    ...
    bool clone(gaussfit_class *);  // FUNCTION
	...
	double		initial_x_pos_start_value;
...
public:
    ...
    bool clone(profile_class *);  // FUNCTION
	...
	bool read_from_simple_txt_FILE(void * file /*FILE rt*/);	// FUNCTION
    bool write_to_simple_txt_FILE(void * file /*FILE wt*/);	// FUNCTION
...
public:
    ...
    void			reset();								// FUNCTION
    bool clone(interpolate_class *);			// FUNCTION
    bool			write_to_ofstream (void * fo /*ofstream*/);	// FUNCTION
    bool			read_from_ifstream(void * fi /*ifstream*/);	// FUNCTION
    ...
    void			smooth_averaged(__int32 number_of_averaged_points);	// FUNCTION
...
//	bool expand_an_hit_array(hit_class**[], __int32 * , __int32); // FUNCTION
    ...
    bool clone(sort_class * clone);  // FUNCTION
...
probably get a slightly different results than the simple COM-calculation.
